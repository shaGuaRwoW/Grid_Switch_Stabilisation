...
% Xj = zeros(m, n, n_of_systems);
% Yj = zeros(n, n, n_of_systems);
% 
% Xl = zeros(n, n, n_of_systems);
% Yl = zeros(n, p, n_of_systems);
...
%     norm(T_A(:, :, i), 'fro')
% 
%     % F_prime = -place(A_matrices(:, :, i), B, 0.9 * [0.96, 0.96, 0.97, 0.99, 0.9, 0.96, 0.96, 0.97, 0.99, 0.9]);
%     % L_prime = -place(A_matrices(:, :, i)', C, 0.9 * [0.96, 0.96, 0.97, 0.99, 0.9, 0.96, 0.96, 0.97, 0.99, 0.9])';
%     % 
%     % K_final = ss(A_matrices(:, :, i) + L_prime * C + B * F_prime, -L_prime, F_prime, 0, 1);
%     % 
%     % K1 = ss(A_ctrl(:, :, i), -L(:, :, i), eye(10), zeros(10, 10), 1);
%     % K2 = ss(A_ctrl(:, :, i), -L(:, :, i), J(:, :, i), zeros(5, 10), 1);
%     % K3 = ss(A_ctrl(:, :, i), B, eye(10), zeros(10, 5), 1);
%     % K4 = ss(A_ctrl(:, :, i), B, J(:, :, i), eye(5), 1);
%     % 
%     % K_p = [K2, K4;
%     %     C * K1 - eye(10), C * K3];
%     % K_final_test = lft(K_p, sys_new);
%     % K_final_test = ss(K_final_test, 'min');
%     % 
%     % dif_final = K_final - K_final_test;
%     % dif_final = ss(dif_final, 'min');
%     % 
%     % norm(dif_final)
% end
% 
